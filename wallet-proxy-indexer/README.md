...
When running the `wallet-proxy-indexer` binary, it will check the current database schema version and execute all remaining database [migrations](./resources/) until the `SchemaVersion::LATEST` is reached to create the
...
This produces a single binary `target/release/wallet-proxy-indexer`.
...
When running the `wallet-proxy-indexer` binary, it will check the current database schema version and execute all remaining database migrations until the `SchemaVersion::LATEST` is reached.
