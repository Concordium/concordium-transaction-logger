...
fn config(node_base_url: &str) -> Cli {
    Cli {
        ...
        node: node_base_url.parse().unwrap(),
        min_connections: 1,
        max_connections: 2,
        statement_timeout_secs: 10,
        listen: SocketAddr::new("0.0.0.0".parse().unwrap(), REST_PORT),
...
const MONITORING_PORT: u16 = 18003;
...
fn start_server_impl() -> ServerHandle {
    ...
    let properties = ServerProperties {
        ...
        node_url: stubs.config.node.uri().to_string(),
    };
    ...
    info!("waiting for wallet proxy service to start");
